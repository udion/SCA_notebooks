...
if __name__=="__main__":
    ...
    crypt = AES_128()
    crypt.key = key
    for q in np.linspace(500000,5000,100):
        pname = 'pt'+str(int(q))+'.txt'
        cname = 'ct'+str(int(q))+'.txt'
        pcname = 'pc'+str(int(q))+'.txt'
        print("file: "+pname)
        #reading the file from the plaintexts
        filer = open("/home/hesl/Desktop/NTU_research/plaintexts/"+pname,'r')
        filew = open("/home/hesl/Desktop/NTU_research/ciphertexts/"+cname,'w')
        filepc = open("/home/hesl/Desktop/NTU_research/pctexts/"+pcname,'w')
        pts = filer.readlines()
        ciphertexts_5000 = np.array([],dtype="S10")
        #counter =0
        for pt in pts:
            p = pt[:-1].decode('hex')
            t = crypt.cipher(p)
            ...
            filew.write(ct+'\n')
            filepc.write(pt[:-1]+","+ct+"\n")
            #ciphertexts_5000 = ciphertexts_5000.reshape((5000,4))
            #print ciphertexts_5000
            #scipy.io.savemat("./ciphertexts/ciphertexts_5000",mdict={"ciphertexts_5000": ciphertexts_5000})
        filew.close()
        filer.close()
